    ...
    def run(self):
        ...
        for global_step in range(self.pipeline_config.max_steps):
            ...
            with (tps_timer, Timer(name="step_total", logger=None) as step_total_timer,
                ...
                # PG MVP: cache use_pg and dp_size for later guards
                use_pg = self.pipeline_config.actor_train.strategy_args.strategy_config.get(
                    "use_prefix_grouper", False
                )
                dp_size = self.actor_train.dp_size
                # Construct continuous prefix_group_id (G=2 runs) for PrefixGrouper
                g = self.pipeline_config.actor_train.strategy_args.strategy_config.get(
                    "prefix_grouper_group_size", 2
                )
                batch.batch["prefix_group_id"] = torch.arange(
                ...
                with Timer(name="step_train", logger=None) as step_train_timer, \
                    ...
                    with actor_train_timer:
                        ...
                        if self.pipeline_config.critic_warmup <= global_step:
                            # PG MVP: skip batch_balance when use_prefix_grouper + dp_size=1
                            use_pg = self.pipeline_config.actor_train.strategy_args.strategy_config.get(
                                "use_prefix_grouper", False
                            )
                            dp_size = self.actor_train.dp_size
                            if not (use_pg and dp_size == 1):
                                batch_balance_metrics = batch_balance(batch, dp_size=dp_size,
                                    ...
                                    * dp_size, logging_prefix="global_seqlen/actor_train")
