...
class FSDP2InferStrategy(FSDP2StrategyBase):
        ...
        with adapter_context:
            for data in micro_batches:
                with torch.autocast(
                    ...
                    # PG MVP: inject prompt_mask, response_mask, prefix_group_id for _fsdp2_forward
                    if self.worker_config.strategy_args.strategy_config.get("use_prefix_grouper", False):
                        forward_args = dict(forward_args)
                        forward_args["prompt_mask"] = data.batch["prompt_mask"]
                        forward_args["response_mask"] = data.batch["response_mask"]
                        forward_args["prefix_group_id"] = data.batch.get("prefix_group_id",
                            torch.full((data.batch.batch_size[0],), -1, device=input_ids.device))
    ...
    def _fsdp2_forward(
        ...
        # PrefixGrouper route — only during training (not compute_log_probs)
        pg_prompt_mask = forward_args.pop("prompt_mask", None)
        pg_response_mask = forward_args.pop("response_mask", None)
        pg_group_id = forward_args.pop("prefix_group_id", None)
        use_pg = (
            self.model.training                          # only train_step, not forward_step
            and pg_prompt_mask is not None
            and pg_response_mask is not None
            and pg_group_id is not None
            and pg_group_id.numel() > 0
            and int(pg_group_id[0].item()) >= 0
        )
        if use_pg:
            import time as _time
            _st = _time.time()
            from roll.utils.prefix_grouper import prefix_grouper_forward_from_data

            result = prefix_grouper_forward_from_data(
                model=self.model,
                data_batch={
                    "input_ids": input_ids,
                    "attention_mask": attention_mask,
                    "prompt_mask": pg_prompt_mask,
                    "response_mask": pg_response_mask,
                    "prefix_group_id": pg_group_id,
                },
                forward_args=forward_args,
                group_size=self.worker_config.strategy_args.strategy_config.get(
                    "prefix_grouper_group_size", 2
                ),
                pad_token_id=self.tokenizer.pad_token_id,
            )
            elapsed = _time.time() - _st
            nz = (result.abs().sum(dim=-1) > 0).sum().item()  # non-zero positions
            total = result.size(0) * result.size(1)
            inp = input_ids.ne(self.tokenizer.pad_token_id)
            print(f"[PG] forward {pg_group_id.tolist()} {elapsed:.3f}s shape={list(result.shape)} non_zero_pos={nz}/{total} inp_active={inp.sum().item()} inp_shape={list(input_ids.shape)}")
            return result
